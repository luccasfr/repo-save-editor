...
/**
 * Decrypts AES-128-CBC encrypted data from a base64 data URI and returns raw buffer
 *
 * @param base64Data - Base64 data URI string containing the encrypted data
 * @param password - The password used for decryption key derivation
 * @returns A buffer containing the decrypted data (automatically unzipped if GZIP compressed)
 */
export async function decryptEs3ToBuffer(
  base64Data: string,
  password: string
): Promise<Buffer> {
  // Extract the base64 part from the data URI if it includes the prefix
  ...
  if (decryptedData.subarray(0, 2).equals(Buffer.from([0x1f, 0x8b]))) {
    return await gunzip(decryptedData);
  }
